...
export async function updateBuildingTransform(
  ...
  return updateMultipleBuildingTransforms([
    { buildingId, position, rotationY },
  ]);
}

export async function updateMultipleBuildingTransforms(
  updates: Array<{
    buildingId: string;
    position: [number, number, number];
    rotationY: number;
  ...
  for (const { buildingId, position, rotationY } of updates) {
    ...
    if (idIndex === -1) {
      ...
      const escapedBuildingId = buildingId.replace(
        /[.*+?^${}()|[\]\\]/g,
        "\\$&",
      );
      const idMatch = new RegExp(`id:\\s*["']${escapedBuildingId}["']`);
      idIndex = content.search(idMatch);
      if (idIndex === -1) continue; // Skip to next if not found
    }
